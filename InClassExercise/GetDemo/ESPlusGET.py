...
# 
# Copyright 2024, Chase Johnson, Tianming Cui, Chai-Wen Hseih,
# Leyan Saleh and Daniel J. Challou 
...
# Add helper function to read file contents
def get_contents(fname):
    with open(fname, 'r') as f:
        return f.read()

# Add helper function to check file permissions
...
class HTTP_GetServer:  #A re-worked version of EchoServer
  ...
  def accept_request(self, client_sock, client_addr):
    ...
    # for a text file once we get a response, 
    # we chop it into utf encoded bytes
    # and send it (like EchoClient)
    # note, if the file you are responding to with a 
    # get request is a binary file, you have to handle 
    # it differently than we do below!!!!
    # You encode the status response line and headers in UTF-8 and 
    # chop them up into bytes and then append the contents of the 
    # binary file and send it off
    # The server we gave you in HW 2 does that!!!!!
  ...
  def process_request(self, request):
    ...
    reqline = linelist[0]
    rlwords = reqline.split() # split on spaces
    if len(rlwords) == 0:
        return ''
  ...
  def get_request(self, resource):
    ...
    if not os.path.exists(resource):
      ...
      ret = header + get_contents(resource)
    return ret

#to do a get request, read resource contents and append to ret value.
#(you should check suffix of the file (resource) before doing so
# to determine whether you are reading a binary or text file)
...
if __name__ == '__main__':
  ...
  HTTP_GetServer(host, port) #Formerly EchoServer
